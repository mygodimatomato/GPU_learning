...
int main() {
    ...
    cudaMemPrefetchAsync(output, dataSize, device);
    ...
    std::cout << "Prefetching - Prefetch Time: " << prefetchTime << " ms, Kernel Time: " << kernelTime << " ms" << std::endl;
    std::cout << "Total bandwidth: " << (dataSize / (prefetchTime + kernelTime)) / 1e6 << " GB/s" << std::endl;
